...
*/
 ...
 ###处理html静态化页面分页的情况###
# total_rows 记录的总数
# method 处理环境 设置为 html 即地址跳转
# parameter 为静态页面地址参数  @符号会被替换为页数
# @问号的位置会自动替换为去向页码 
...
$params = array(
            'total_rows'=>100, #(必须)
            'method'    =>'html', #(必须)
            'parameter' =>'http://magic_vshop/web/index.php?c=site&gop=@',  #(必须)
            'now_page'  =>$_GET['gop'],  #(必须)
            'list_rows' =>10, #(可选) 默认为15
);
$page = new LibPage($params);
echo  $page->show(1);
#<a href="http://magic_vshop/web/index.php?c=site&gop=2">2</a>
...
#添加了parameter<a href="javascript:void(0)" onclick="goToPage('6','fky','username')">6</a>
