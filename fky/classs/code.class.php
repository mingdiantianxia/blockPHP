...
class Code{
	...
	//创建验证码
	public function make($verifyName = 'code')
	{
		if(empty($this->font))
		{
			$this->font = __DIR__.'/../inc/code/consola.ttf';
		}
		$this->create($verifyName);//生成验证码
	...
	//返回验证码
	public function get($verifyName = 'code', $clean = false) {
		$verifycode = $_SESSION[$verifyName];
		if ($clean) {
	...
	//生成验证码
	private function createCode($verifyName = 'code') {
		$code = '';
		for ($i = 0; $i < $this->codeLen; $i++) {
			$code .= $this->codeStr [mt_rand(0, strlen($this->codeStr) - 1)];
		}
		$this->code = strtoupper($code);
		$_SESSION[$verifyName] = $this->code;
	}

	//建画布
	private function create($verifyName = 'code') {
		if (!$this->checkGD())
			return false;
		$w = $this->width;
		$h = $this->height;
	...
	//写入验证码文字
	private function createFont($verifyName = 'code') {
		$this->createCode($verifyName);
