...
class Image {
    ...
    static function buildImageVerify($length=4, $mode=1, $type='png', $width=48, $height=22, $verifyName='verify') {
        require 'fString.class.php';
        $randval = Fstring::randString($length, $mode);
        @session_start();
    ...
    static function GBVerify($length=4, $type='png', $width=180, $height=50, $fontface='simhei.ttf', $verifyName='verify') {
        require 'fString.class.php';
        $code = Fstring::randString($length, 4);
        ...
        if (!is_file($fontface)) {
            $fontface = __DIR__.'/../inc/image/' . $fontface;
        }
        for ($i = 0; $i < $length; $i++) {
            $fontcolor = imagecolorallocate($im, mt_rand(0, 120), mt_rand(0, 120), mt_rand(0, 120)); //这样保证随机出来的颜色较深。
            $codex = Fstring::msubstr($code, $i, 1);
