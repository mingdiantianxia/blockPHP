...
class PHPWebSocket
{
	...
	*/
	...
	public $left;
	public $join;
	public function  __construct($said = null,$join = null,$left = null){
		$this->said = $said;
		$this->join = $join;
		$this->left = $left;
	...
	function wsStartServer($host = '127.0.0.1', $port = 9300) {
		...
		elseif ($opcode == self::WS_OPCODE_TEXT || $opcode == self::WS_OPCODE_BINARY) {
			if ( array_key_exists('message', $this->wsOnEvents) )
				foreach ( $this->wsOnEvents['message'] as $func )
					if(!method_exists(__CLASS__ ,$func)){
	    				$function = 'fky\\'.$func;
						$function($clientID, $data, $dataLength, $opcode == self::WS_OPCODE_BINARY);
					 } else {
...
//自定义处理的函数
	...
	function wsOnMessage($clientID, $message, $messageLength, $binary) {
		...
		if ( sizeof($this->wsClients) == 1 )
			$this->wsSend($clientID, "There isn't anyone else in the room, but I'll still listen to you.");
		else
			//Send the message to everyone but the person who said it
			foreach ($this->wsClients as $id => $client )
				if ( $id != $clientID )
					$this->wsSend($id, $this->said?$this->said:"$message");
					// $this->wsSend($id, $this->said?$this->said:"Visitor $clientID ($ip) said \"$message\"");
	...
	function wsOnOpen($clientID)
	{
		...
		foreach ( $this->wsClients as $id => $client )
			if ( $id != $clientID )
				...
				$this->wsSend($id, $this->join?$this->join:"Visitor $clientID ($ip) has joined the room.");
	}
	...
	function wsOnClose($clientID, $status) {
		...
		foreach ( $this->wsClients as $id => $client )
			...
			$this->wsSend($id, $this->left?$this->left:"Visitor $clientID ($ip) has left the room.");
	}
